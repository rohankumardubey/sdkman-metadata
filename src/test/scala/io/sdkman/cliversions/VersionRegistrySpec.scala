...
class VersionRegistrySpec extends AsyncWordSpec with AsyncIOSpec with EmbeddedMongo with Matchers:
  ...
  private def getActiveVersions(path: Path): IO[Response[IO]] = {
    MongoClient.fromConnectionString[IO](s"mongodb://localhost:$mongoPort").use { (client: MongoClient[IO]) =>
      ...
      for {
        ...
        application = Document("stableCliVersion", "5.15.0").append("betaCliVersion", "latest+9f86a55").append("stableNativeCliVersion", "0.0.15")
        _ <- collection.insertOne(application)
        registry: VersionRegistry[IO] = VersionRegistry.impl[IO](db)
        response: Response[IO] <- VersionRegistryRoutes.activeVersionsRoutes(registry).orNotFound(request)
      } yield response
    }
  }
